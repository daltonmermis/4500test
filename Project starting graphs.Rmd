...
main_data <- loan_long %>% filter(Group %in% main_groups)
extra_data <- loan_long %>% filter(!(Group %in% main_groups))

p_1 <- ggplot() +
  geom_boxplot(data = main_data, aes(x = Group, y = DefaultRate, fill = Group), alpha = 0.7) +
  geom_jitter(data = extra_data, 
              aes(x = "Other", y = DefaultRate, text = paste("Group:", Group, "<br>Default Rate:", percent(DefaultRate, 0.1))), 
